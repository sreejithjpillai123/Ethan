...
document.addEventListener("DOMContentLoaded" , ()=>{
    ...
    ScrollTrigger.create({
        ...
        Object.entries(cubesData).forEach(([cubeClass, data],index)=>{
            console.log(`🟢 Iteration ${index + 1}: Processing ${cubeClass}`);
            ...
            if (cube.length === 0) {
                console.warn(`❌ No elements found for class: ${cubeClass}`);
                return; // Skip iteration if no elements found
            }


            const {initial ,final } = data;
            console.log(initial.top)
...
console.log(currentTop)
    ...
    console.log(`Cube: ${cubeClass}, RotateY: ${currentRotateY + additionalRotation}`);
